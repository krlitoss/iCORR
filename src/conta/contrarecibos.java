 ...
 * @author IVONNE
 */
public class contrarecibos extends javax.swing.JInternalFrame {
    ...
    public contrarecibos(Connection connt) {
        initComponents();
    ...
    public void datos_privilegios(){
        valor_privilegio=conexion.obtener_privilegios(conn,"Contrarecibos");
    }
    ...
    public void datos(){
        rs0=null;
        try{
            String senSQL="SELECT contrarecibos.*, clientes.nombre, importe.sumasaldos AS importe_total FROM contrarecibos LEFT JOIN clientes ON contrarecibos.id_clientes = clientes.id_clientes LEFT JOIN ( SELECT contrarecibos_detalle.id_contrarecibo, SUM ( contrarecibos_detalle.saldo ) AS sumasaldos FROM contrarecibos_detalle GROUP BY contrarecibos_detalle.id_contrarecibo ) AS importe ON contrarecibos.id_contrarecibo = importe.id_contrarecibo WHERE ( contrarecibos.estatus <> 'Can' AND ( contrarecibos.fecha >= '"+conf.getProperty("FechaIni")+" 00:00:00' AND contrarecibos.fecha <= '"+conf.getProperty("FechaFin")+" 23:59:59' )) ORDER BY contrarecibos.fecha;";
            rs0=conexion.consulta(senSQL,conn);
            while(rs0.next()){
                Object datos[]={rs0.getString("id_contrarecibo"),fechacorta.format(rs0.getDate("fecha")),rs0.getString("nombre"),rs0.getString("contrarecibo"),fechacorta.format(rs0.getDate("fecha_pago")), moneda2decimales.format(rs0.getDouble("importe_total"))};
    ...
    public void modificar() {
        ...
        if (filano < 0) {
            ...
            if (clavemodifica.equals("")) {
                ...
                datos_contrarecibos = new datos_contrarecibos(null, true, conn, clavemodifica,valor_privilegio);
                datos_contrarecibos.setLocationRelativeTo(this);
                datos_contrarecibos.setVisible(true);
                datos_contrarecibos = null;
            }
        }
    }
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(conta.CONTAApp.class).getContext().getResourceMap(contrarecibos.class);
        ...
        Tabladatos.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Fecha", "Cliente", "Contrarecibo", "Fecha Pago", "Importe Total"
            }
        ) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (estado.equals("cancelar")) {
            ...
            rs0=null;
            try{
                String senSQL="SELECT contrarecibos.*,clientes.nombre FROM contrarecibos LEFT JOIN clientes ON contrarecibos.id_clientes=clientes.id_clientes WHERE (contrarecibos.estatus<>'Can' AND (contrarecibos.fecha>='"+fechainsertar.format(busca_fechas.getFechaini())+" 00:00:00' AND contrarecibos.fecha<='"+fechainsertar.format(busca_fechas.getFechafin())+" 23:59:59')) ORDER BY contrarecibos.fecha;";
                rs0=conexion.consulta(senSQL,conn);
                while(rs0.next()){
                    Object datos[]={rs0.getString("id_contrarecibo"),fechacorta.format(rs0.getDate("fecha")),rs0.getString("nombre"),rs0.getString("contrarecibo"),fechacorta.format(rs0.getDate("fecha_pago"))};
                    modelot1.addRow(datos);
                }
                if(rs0!=null) {  rs0.close(); }
            } catch (Exception ex) {JOptionPane.showMessageDialog(this,"ERROR AL EJECUTAR LA CONSULTA\n"+ ex,"E R R O R !!!!!!!!!!",JOptionPane.ERROR_MESSAGE);      }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        // TODO add your handling code here:
        datos_contrarecibos = new datos_contrarecibos(null,true,conn,"",valor_privilegio);
        datos_contrarecibos.setLocationRelativeTo(this);
        datos_contrarecibos.setVisible(true);
        limpiatabla();
        datos_contrarecibos=null;
    ...
    private JDialog datos_contrarecibos;
}
