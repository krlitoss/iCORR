 ...
 * @author ANGEL
 */
public class busca_ops_juegos extends javax.swing.JDialog {
    Connection connj;
    ...
    public busca_ops_juegos(java.awt.Frame parent, boolean modal,Connection conn,String clavemodifica,String idop,String valor_privilegio_t) {
        super(parent, modal);
        initComponents();
                ...
                try{
                    ...
                    if (c == 4) {
                        if(model.getValueAt(f, c) != null){
                            ...
                            if(preciomt>0.0){
                                actualizaPrecioOP(preciomt,(""+model.getValueAt(f, 0)),(""+model.getValueAt(f, 1)) );
                            }
                        }
                        sumatoria();
                    }
    ...
    public void datos(String idop){
        rs0=null;
        try{
            String senSQL="SELECT ops.*,articulos.articulo,articulos.claveresistencia,articulos.preciomillar,articulos.kg,pedidos_detalle.cantidad as cantidadjgo FROM (ops LEFT JOIN articulos ON ops.clavearticulo=articulos.clavearticulo) LEFT JOIN pedidos_detalle ON ops.id_op=pedidos_detalle.id_op WHERE ops.id_op='"+idop+"';";
            rs0=conexion.consulta(senSQL,connj);
            while(rs0.next()){
                Double kg=rs0.getDouble("kg");
                Double pu=rs0.getDouble("preciounitario");
                Double pzasjgo=rs0.getDouble("cantidad") / rs0.getDouble("cantidadjgo");
                Object datos[]={rs0.getString("op"),rs0.getString("clavearticulo"),rs0.getString("articulo"),rs0.getString("claveresistencia"),pu,pzasjgo,kg,moneda2decimales.format(pu/kg)};
        ...
        sumatoria();
        if(clavebusca<=0.0){
            JOptionPane.showMessageDialog(this,"NO SE ENCONTRARON OPS JUEGOS","E R R O R !!!!!!!!!!",JOptionPane.ERROR_MESSAGE);
        }
    ...
    public void sumatoria(){
        ...
        if(filas<=0){
            ...
            for(int i=0;i<=(filas-1);i=i+1){
                Double valpiezas=(Double) Tabladatos.getValueAt(i, 5);///obtiene el importe
    ...
    public void actualizaPrecioOP(Double pmt,String op,String art){
        String senSQLmov="INSERT INTO bitacora_op(usuario, fecha, op, clavearticulo, descripcion) VALUES ('"+user+"', '"+fechainsertarhora.format(new Date())+"', '"+op+"', '"+art+"', 'Modificacion de Precio por: "+pmt+"');";
        conexion.modificamov_p(senSQLmov,connj,valor_privilegio);
        senSQLmov="UPDATE ops SET preciounitario='"+pmt+"' WHERE op='"+op+"';";
        conexion.modificamov_p(senSQLmov,connj,valor_privilegio);
        System.err.println("-Actualizado-"+op+":"+pmt);////////////////////////
    }
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(conta.CONTAApp.class).getContext().getResourceMap(busca_ops_juegos.class);
        ...
        Tabladatos.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "OP", "Clave Articulo", "Articulo", "Resistencia", "Precio Unitario", "Piezas x Juego", "Peso Kg", "P.K."
            }
        ) {
