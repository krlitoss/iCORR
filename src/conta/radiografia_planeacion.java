 ...
 * @author IVONNE
 */
public class radiografia_planeacion extends javax.swing.JInternalFrame {
    ...
    public radiografia_planeacion(Connection connt) {
        initComponents();
    ...
    public void ajusteTabladatos() {
        ...
        Tabla_horas.getColumnModel().getColumn(2).setPreferredWidth(80);
        Tabla_horas.getColumnModel().getColumn(3).setPreferredWidth(80);
        Tabla_horas.getColumnModel().getColumn(4).setPreferredWidth(80);
        Tabla_horas.getColumnModel().getColumn(5).setPreferredWidth(80);
        Tabla_horas.getColumnModel().getColumn(6).setPreferredWidth(100);
        Tabla_horas.getColumnModel().getColumn(7).setPreferredWidth(100);
        ...
        Tabla_horas.getColumnModel().getColumn(8).setCellRenderer(new DoubleRenderer());
        Tabla_horas.getColumnModel().getColumn(9).setCellRenderer(new DoubleRenderer());
    ...
    public void grafica_horas(String fi,String ft,String sqlclavecli,String save){
        ...
        if(!sqlclavecli.equals("")){
            sqlcliacum=" AND pendientes."+sqlclavecli;
            sqlcliacum2=" AND ops."+sqlclavecli;
        }
        //resul en null
        modelot_horas.setNumRows(0);
        ...
        rs0=null;
        try{
            String senSQL="SELECT total.clave,max(total.desmaquina) as desmaquina,max(total.nombre) as nombrecli,sum(total.arreglo) as suma_arreglo,sum(total.cantidadpendiente/total.velocidad) as suma_produccion,sum(total.cantidadpendiente_maq/total.velocidad) as suma_produccion_maq,count(total.clave) as cuenta_cambios,sum(total.cantidadpendiente) as cantidadpendiente,sum(total.cantidadpendiente_maq) as cantidadpendiente_maq,sum(total.kgpendiente) as kg,sum(total.m2pendiente) as m2 FROM (SELECT articulos_maquinas.*, pendientes.*,maquinas.descripcion as desmaquina FROM (articulos_maquinas LEFT JOIN maquinas ON articulos_maquinas.clave=maquinas.clave) INNER JOIN (SELECT ops.*,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))) as cantidadpendiente,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))/articulos.piezas) as cantidadpendiente_maq,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))*articulos.kg) as kgpendiente,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))*articulos.m2) as m2pendiente,COALESCE(remisionado.cantidadremision,0) as cantidadremisionado,remisionado.fecharemisionado,articulos.articulo,articulos.claveresistencia,articulos.largo,articulos.ancho,articulos.diseno,articulos.piezas,articulos.kg,articulos.m2,clientes.nombre,agentes.id_agente,agentes.nombre as nombreagente,COALESCE(almacen_pt.cantidadpzas,0) as cantidadinventario FROM (((ops  LEFT JOIN articulos ON ops.clavearticulo=articulos.clavearticulo) LEFT JOIN (clientes LEFT JOIN agentes ON clientes.id_agente=agentes.id_agente) ON ops.id_clientes=clientes.id_clientes ) LEFT JOIN almacen_pt ON ops.op=almacen_pt.op) LEFT JOIN (SELECT remisiones_detalle.op,sum(remisiones_detalle.cantidadpzas) as cantidadremision,max(remisiones.fechahora) as fecharemisionado FROM remisiones_detalle LEFT JOIN remisiones ON remisiones_detalle.remision=remisiones.remision WHERE (remisiones_detalle.estatus='Act') GROUP BY remisiones_detalle.op) as remisionado ON ops.op=remisionado.op WHERE ( (ops.fechaentrega>='"+fi+" 00:00:00' AND ops.fechaentrega<='"+ft+" 23:59:59') AND ops.estatus<>'Can' AND ops.estatus<>'Ter' ) ORDER BY ops.fechaentrega )as pendientes ON articulos_maquinas.clavearticulo=pendientes.clavearticulo WHERE ((articulos_maquinas.clave='TCY' OR articulos_maquinas.clave='MACARB' OR articulos_maquinas.clave='COM' OR articulos_maquinas.clave='SUAP' OR articulos_maquinas.clave='DIEC' OR articulos_maquinas.clave='RAY')"+sqlcliacum+")) total GROUP BY total.clave;";
            rs0=conexion.consulta(senSQL,conn);
            while(rs0.next()){
                String clavemaquina=rs0.getString("clave");
                Double arreglo=rs0.getDouble("suma_arreglo");
                Double prod=rs0.getDouble("suma_produccion_maq");
                Double cant_pen=rs0.getDouble("cantidadpendiente");
                Double cant_pen_maq=rs0.getDouble("cantidadpendiente_maq");
                Double cuenta_cambios=rs0.getDouble("cuenta_cambios");
                ...
                Object datos[]={rs0.getString("clave"),rs0.getString("desmaquina"),Integer.parseInt(fijo0decimales.format(cant_pen_maq)),Integer.parseInt(fijo0decimales.format(cant_pen)),rs0.getInt("m2"),rs0.getInt("kg"),cadena_arreglo,cadena_prod,cadena_total,Integer.parseInt(fijo0decimales.format(cuenta_cambios)),Integer.parseInt(fijo0decimales.format(arreglo/cuenta_cambios)),Integer.parseInt(fijo0decimales.format(cant_pen_maq/prod))};
                modelot_horas.addRow(datos);
            }
            if(rs0!=null) {     rs0.close();    }
        } catch (Exception ex) {JOptionPane.showMessageDialog(this,"ERROR AL EJECUTAR LA CONSULTA\n"+ ex,"E R R O R !!!!!!!!!!",JOptionPane.ERROR_MESSAGE);      }
        ...
        rs0=null;
        try{
            String senSQL="SELECT total.clave,max(total.desmaquina) as desmaquina,max(total.nombre) as nombrecli,sum(total.arreglo) as suma_arreglo,sum(total.cantidadpendiente/total.velocidad) as suma_produccion,sum(total.cantidadpendiente_maq/total.velocidad) as suma_produccion_maq,count(total.clave) as cuenta_cambios,sum(total.cantidadpendiente) as cantidadpendiente,sum(total.cantidadpendiente_maq) as cantidadpendiente_maq,sum(total.kgpendiente) as kg,sum(total.m2pendiente) as m2 FROM (SELECT articulos_maquinas.*, pendientes.*,maquinas.descripcion as desmaquina FROM (articulos_maquinas LEFT JOIN maquinas ON articulos_maquinas.clave=maquinas.clave) INNER JOIN (SELECT ops.*,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))) as cantidadpendiente,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))/articulos.piezas) as cantidadpendiente_maq,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))*articulos.kg) as kgpendiente,((ops.cantidad-COALESCE(remisionado.cantidadremision,0))*articulos.m2) as m2pendiente,COALESCE(remisionado.cantidadremision,0) as cantidadremisionado,remisionado.fecharemisionado,articulos.articulo,articulos.claveresistencia,articulos.largo,articulos.ancho,articulos.diseno,articulos.piezas,articulos.kg,articulos.m2,clientes.nombre,agentes.id_agente,agentes.nombre as nombreagente,COALESCE(almacen_pt.cantidadpzas,0) as cantidadinventario FROM (((ops  LEFT JOIN articulos ON ops.clavearticulo=articulos.clavearticulo) LEFT JOIN (clientes LEFT JOIN agentes ON clientes.id_agente=agentes.id_agente) ON ops.id_clientes=clientes.id_clientes ) LEFT JOIN almacen_pt ON ops.op=almacen_pt.op) LEFT JOIN (SELECT remisiones_detalle.op,sum(remisiones_detalle.cantidadpzas) as cantidadremision,max(remisiones.fechahora) as fecharemisionado FROM remisiones_detalle LEFT JOIN remisiones ON remisiones_detalle.remision=remisiones.remision WHERE (remisiones_detalle.estatus='Act') GROUP BY remisiones_detalle.op) as remisionado ON ops.op=remisionado.op WHERE ( (ops.fechaentrega>='"+fi+" 00:00:00' AND ops.fechaentrega<='"+ft+" 23:59:59') AND ops.estatus<>'Can' AND ops.estatus<>'Ter' ) ORDER BY ops.fechaentrega )as pendientes ON articulos_maquinas.clavearticulo=pendientes.clavearticulo WHERE ((articulos_maquinas.clave='PLTCY' OR articulos_maquinas.clave='PLMACARB' OR articulos_maquinas.clave='ENG' OR articulos_maquinas.clave='MARVI' OR articulos_maquinas.clave='PDO' OR articulos_maquinas.clave='DES' OR articulos_maquinas.clave='MAN' OR articulos_maquinas.clave='FON' OR articulos_maquinas.clave='FLE' OR articulos_maquinas.clave='TRO')"+sqlcliacum+")) total GROUP BY total.clave;";
            rs0=conexion.consulta(senSQL,conn);
            while(rs0.next()){
                String clavemaquina=rs0.getString("clave");
                Double arreglo=rs0.getDouble("suma_arreglo");
                ...
                Object datos[]={rs0.getString("clave"),rs0.getString("desmaquina"),Integer.parseInt(fijo0decimales.format(cant_pen)),Integer.parseInt(fijo0decimales.format(cant_pen)),rs0.getInt("m2"),rs0.getInt("kg"),cadena_arreglo,cadena_prod,cadena_total,Integer.parseInt(fijo0decimales.format(cuenta_cambios)),Integer.parseInt(fijo0decimales.format(arreglo/cuenta_cambios)),Integer.parseInt(fijo0decimales.format(cant_pen/prod))};
                modelot_horas.addRow(datos);
            }
            if(rs0!=null) {     rs0.close();    }
        } catch (Exception ex) {JOptionPane.showMessageDialog(this,"ERROR AL EJECUTAR LA CONSULTA\n"+ ex,"E R R O R !!!!!!!!!!",JOptionPane.ERROR_MESSAGE);      }
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(conta.CONTAApp.class).getContext().getResourceMap(radiografia_planeacion.class);
        ...
        Tabla_horas.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Clave Maquina", "Descripcion", "Cantidad", "Piezas", "M2", "KG", "Total Arreglo", "Total Produccion", "Total Tiempo", "OP's", "Avg_arreglo", "Avg_vel."
            }
        ) {
            Class[] types = new Class [] {
                java.lang.String.class, java.lang.String.class, java.lang.Integer.class, java.lang.Integer.class, java.lang.Integer.class, java.lang.Integer.class, java.lang.String.class, java.lang.String.class, java.lang.String.class, java.lang.Integer.class, java.lang.Integer.class, java.lang.Integer.class
            };
