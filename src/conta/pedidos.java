...
public class pedidos extends javax.swing.JInternalFrame {
    ...
    public pedidos(Connection connt) {
        initComponents();
    ...
    public void datos_privilegios(){
        valor_privilegio=conexion.obtener_privilegios(conn,"Pedidos");
    }
    public void ajusteTabladatos() {
        ...
        Tabladatos.getColumnModel().getColumn(2).setPreferredWidth(280);
    ...
    public void datos(){
        rs0=null;
        try{
            String senSQL="SELECT pedidos.*, articulos.destino, clientes.nombre FROM pedidos LEFT JOIN clientes ON pedidos.id_clientes = clientes.id_clientes LEFT JOIN pedidos_detalle ON pedidos.id_pedido = pedidos_detalle.id_pedido INNER JOIN articulos ON articulos.clavearticulo = pedidos_detalle.clavearticulo WHERE((pedidos.fecha>='"+conf.getProperty("FechaIni")+" 00:00:00' AND pedidos.fecha<='"+conf.getProperty("FechaFin")+" 23:59:59') and pedidos.estatus='Act') ORDER BY pedidos.id_pedido ";
            rs0=conexion.consulta(senSQL,conn);
            while(rs0.next()){
                Object datos[]={rs0.getString("id_pedido"),fechamascorta.format(rs0.getDate("fecha")),rs0.getString("nombre"),rs0.getString("ordencompra"),rs0.getString("destino"),rs0.getInt("partidas"),moneda2decimales.format(rs0.getDouble("subtotal"))};
    ...
    public void modificar(){
        ...
        if(filano<0){
            ...
            if(clavemodifica.equals("")){
                ...
                datos_pedidos = new datos_pedidos(null,true,conn,clavemodifica,valor_privilegio);
                datos_pedidos.setLocationRelativeTo(this);
                datos_pedidos.setVisible(true);
                limpiatabla();
                datos_pedidos=null;
                datos();
            }
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(conta.CONTAApp.class).getContext().getResourceMap(pedidos.class);
        ...
        Tabladatos.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Fecha", "Cliente", "Orden Compra", "Destino", "Partidas", "Subtotal"
            }
        ) {
        ...
        Tabladatos.getColumnModel().getColumn(4).setHeaderValue(resourceMap.getString("Tabladatos.columnModel.title6")); // NOI18N
        Tabladatos.getColumnModel().getColumn(5).setHeaderValue(resourceMap.getString("Tabladatos.columnModel.title4")); // NOI18N
        Tabladatos.getColumnModel().getColumn(6).setHeaderValue(resourceMap.getString("Tabladatos.columnModel.title5")); // NOI18N
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        // TODO add your handling code here:
        datos_pedidos = new datos_pedidos(null,true,conn,"",valor_privilegio);
        datos_pedidos.setLocationRelativeTo(this);
        datos_pedidos.setVisible(true);
        limpiatabla();
        datos_pedidos=null;
            ...
            if (respuesta == JOptionPane.YES_OPTION) {
                ...
                if(validaopremision==0){
                        ...
                        if (!motivocancela.equals("") && !motivocancela.equals("null")) {
                            try{
                                senSQL="INSERT INTO bitacora(usuario, fecha, descripcion,host) VALUES ('"+conf.getProperty("UserID")+"', '"+fechainsertarhora.format(new Date())+"', 'Cancela Pedido ("+clavepedido+"): "+motivocancela+"','"+java.net.InetAddress.getLocalHost().getCanonicalHostName()+"');";
                                conexion.modificamov_p(senSQL,conn,valor_privilegio);
                            }catch(Exception e){  }

                            senSQL="UPDATE ops SET estatus='Can' WHERE id_pedido='"+clavepedido+"';";
                            conexion.modificamov_p(senSQL,conn,valor_privilegio);
                            senSQL="UPDATE pedidos SET estatus='Can' WHERE id_pedido='"+clavepedido+"';";
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        if(estado.equals("cancelar")){
            ...
            if(busca_fechascliente.getProveedor().equals("Buscar")){ /**verifica si buscas un solo proveedor o todos*/
                dinamicoSQL="AND pedidos.id_clientes='"+busca_fechascliente.getID()+"'";
            }

            limpiatabla();
            rs0=null;
            try{
                String senSQL="SELECT pedidos.*,clientes.nombre FROM pedidos LEFT JOIN clientes ON pedidos.id_clientes=clientes.id_clientes WHERE ((pedidos.fecha>='"+fechainsertar.format(busca_fechascliente.getFechaini())+" 00:00:00' AND pedidos.fecha<='"+fechainsertar.format(busca_fechascliente.getFechafin())+" 23:59:59') and estatus='Act' "+dinamicoSQL+") ORDER BY id_pedido";
                rs0=conexion.consulta(senSQL,conn);
                while(rs0.next()){
                    Object datos[]={rs0.getString("id_pedido"),fechamascorta.format(rs0.getDate("fecha")),rs0.getString("nombre"),rs0.getString("ordencompra"),rs0.getInt("partidas"),moneda2decimales.format(rs0.getDouble("subtotal"))};
                    modelot1.addRow(datos);
                }
                if(rs0!=null) {
                    rs0.close();
